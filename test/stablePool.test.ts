...
describe('StablePool', () => {
  ...
  describe('fetcher', () => {
    it('constructor test', async () => {
      ...
      const stablePool = new StablePool(STABLES_INDEX_MAP[chainId], tokenReserves, _A, swapStorage, blockNumber)
      ...
      const inIndex = 0
      const outIndex = 3
      const swapActual = await stablePool.calculateSwapViaIndex(inIndex, outIndex, '100', jsonProv)

      console.log("calculate swap", swapActual.toString())
      const inAmount = BigNumber.from('100')
      const swapManual = stablePool.calculateSwapFromData(inIndex, outIndex, inAmount)
      ...
      //   wallet.)
      // console.log(data)
      let x = BigNumber.from(2)
      const y = x.mul(BigNumber.from(0))
      console.log("x", x, "y", y)
    }
    )

...
import { WeightedPair } from '../src/entities/weightedPair'
...
describe('TradeV4', () => {
  ...
  describe('#bestTradeExactIn', () => {
    it('throws with empty pools', () => {
      expect(() => TradeV4.bestTradeExactIn(stablePool, [], new TokenAmount(token0, JSBI.BigInt(100)), token2)).toThrow('PAIRS')
    })
    it('throws with max hops of 0', () => {
      expect(() =>
        TradeV4.bestTradeExactIn(stablePool, [pair_t0_t2], new TokenAmount(token0, JSBI.BigInt(100)), token2, { maxHops: 0 })
      ).toThrow('MAX_HOPS')
    })
    ...
    it('doesnt throw for zero liquidity pools', () => {
      expect(TradeV4.bestTradeExactIn(stablePool, [empty_pair_0_1], new TokenAmount(token0, JSBI.BigInt(100)), token1)).toHaveLength(
        0
      )
    })
  ...
  describe('#bestTradeExactOut', () => {
    it('throws with empty pools', () => {
      expect(() => TradeV4.bestTradeExactOut(stablePool, [], token0, new TokenAmount(token2, JSBI.BigInt(100)))).toThrow('PAIRS')
    })
    it('throws with max hops of 0', () => {
      expect(() =>
        TradeV4.bestTradeExactOut(stablePool, [pair_t0_t2], token0, new TokenAmount(token2, JSBI.BigInt(100)), { maxHops: 0 })
      ).toThrow('MAX_HOPS')
    })
    ...
    it('doesnt throw for zero liquidity pools', () => {
      expect(TradeV4.bestTradeExactOut(stablePool, [empty_pair_0_1], token1, new TokenAmount(token1, JSBI.BigInt(100)))).toHaveLength(
        0
      )
    })

...
export function getTotalValue(pair: WeightedPair, reqt: Token): BigNumber {
    ...
    const [reservesOther, weightReqt, weightOther] = reqt.equals(pair.token0)
        ? [reserve1, pair.weight0, pair.weight1]
        : [reserve0, pair.weight1, pair.weight0]

    return SQRT2x100.mul(reservesOther.toBigNumber()).div(
        sqrrt(BigNumber.from(JSBI.add(JSBI.multiply(weightOther, weightOther), JSBI.multiply(weightReqt, weightReqt)).toString())
        )).div(ONE_E18)
}
...
export function markdown(pair: WeightedPair, reqt: Token): BigNumber {
    const [reservesOther, weightOther, weightReqt] = reqt.equals(pair.token0)
    ...
    return reservesOther.add(
        weightOther.mul(reservesOther).div(weightReqt)).mul(
            BigNumber.from(JSBI.exponentiate(TEN, JSBI.BigInt(reqt.decimals))).div(
                getTotalValue(pair, reqt)
            )
        )
}